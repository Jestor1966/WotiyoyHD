...
public class EditorPanelElement extends InterfaceElement<EditorPanelElement> {
    ...
    public void initPiecesButtons() {
        buttons.clear();
        addBackButton();
        float y = position.height - 3 * bRadius;
        ...
        for (EpbType type : types) {
            ...
            if(type==EpbType.mountain){
                x = position.width - hDelta - bRadius;
                y += 2 * bRadius;
            }
    ...
    private boolean isPieceTypeAllowed(EpbType epbType) {
        ...
        if (epbType == EpbType.mountain) return true;
        if (epbType == EpbType.fort) return true;
        if (epbType == EpbType.town) return true;
